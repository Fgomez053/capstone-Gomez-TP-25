...
def reset_result_bg(result_text):
    """Reset the result_text background for the current theme."""
    mode = ctk.get_appearance_mode()
    result_text.configure(fg_color="#1a1a1a" if mode=="Dark" else "#ffffff")
...
def start_app():
    ...
    # ── Top area: icon + text side by side ────────────────────────
    display_frame = ctk.CTkFrame(root)
    display_frame.pack(pady=10, padx=20, fill="both", expand=True)
    ...
    tab_main    = tab_view.add("Main")
    tab_tracker = tab_view.add("City Tracker")
    tab_group3  = tab_view.add("Group 3")
    create_group_feature_tab(tab_group3)
    ...
    def on_fetch():
        ...
        try:
            ...
            messagebox.showerror("Fetch Error", "Check spelling or network.")
            return
        ...
        activity = suggest_activity(data.get("weather",[{}])[0].get("main",""))
        text += f"\n\n🌟 Suggested Activity:\n{activity}"

        result_text.configure(state="normal")
        result_text.delete("0.0","end")
        result_text.insert("0.0", text)
        result_text.configure(state="disabled")
    ...
    # Recall buttons for last 3/5/7 days
    def _show_tracked_days(days):
        history = load_tracked_history(days)
        if not history:
        ...
        for row in history:
            ...
            result_text.insert(
                "end",
                f"{ts} — {row['city']} — {row['temp_F']}°F, {row['humidity']}% humidity\n"
            )
        result_text.configure(state="disabled")
    ...
    for days in (3, 5, 7):
        ctk.CTkButton(
            ...
            width=140,
            command=lambda d=days: _show_tracked_days(d)
