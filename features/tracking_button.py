...
def create_tracking_button(parent, entry_city, result_text):
    def on_track_click():
        ...
        if not city:
            messagebox.showwarning("Input Error", "Please enter a city to track.")
            return
        try:
        ...
        messagebox.showinfo("Success", f"Started tracking {city}.")

        # preview
        result_text.configure(state="normal")
        result_text.delete("0.0","end")
        result_text.insert("0.0", f"Tracked {city} at {data['main']['temp']}°F\n")
